...
\subsection*{Example: Beta Posterior Form}

Flat prior: \( f(\theta) = 1 \). \\
Data: 27 tosses, 15 heads, 12 tails. 

Posterior:
\(
f(\theta|x) = \frac{\theta^{15}(1 - \theta)^{12}}{T}
\)
where
\(
T = \int_0^1 \theta^{15}(1 - \theta)^{12}\,d\theta
\)
Hence posterior is \( \text{Beta}(16, 13) \). 
...
\)
Thus \( f(\theta) \) is the pdf of \( \text{Beta}(a, b) \).

\textit{Example:}  
If posterior \( f(\theta|x) = 3\theta^2(1 - \theta) \),  
then \( a = 3, b = 2 \Rightarrow \text{Beta}(3,2) \). 
